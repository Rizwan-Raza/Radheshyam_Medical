package com.cspoint.radheshyam.utiles;
...
import static com.cspoint.radheshyam.ui.HomeActivity.txtCountcard;
...
public class DatabaseHelper extends SQLiteOpenHelper {
    ...
    public boolean insertData(MyCart rModel) {
        if (getPID(rModel.getPid()) == -1) {
            ...
            return changeData(rModel.getPid(), rModel.getQty());
        }
    }
    ...
    private int getPID(String mid) {
        SQLiteDatabase db = this.getWritableDatabase();
        Cursor c = db.query(TABLE_NAME, new String[]{"PID"}, "PID =? ", new String[]{mid}, null, null, null, null);
        if (c.moveToFirst()) //if the row exist then return the id
            return c.getInt(c.getColumnIndex("PID"));
        return -1;
    }
    ...
    public int getCart(String mid) {
        SQLiteDatabase db = this.getWritableDatabase();
        Cursor c = db.query(TABLE_NAME, new String[]{"qty"}, "PID=? ", new String[]{mid}, null, null, null, null);
        if (c.moveToFirst()) { //if the row exist then return the id
            return c.getInt(c.getColumnIndex("qty"));
        } else {
            return -1;
    ...
    public boolean changeData(String mid, String qty) {
        SQLiteDatabase db = this.getWritableDatabase();
        ContentValues contentValues = new ContentValues();
        contentValues.put(ICOL_8, qty);
        db.update(TABLE_NAME, contentValues, "PID = ? ", new String[]{mid});
        Cursor res = getAllData();
        txtCountcard.setText("" + res.getCount());
        return true;
    }
    ...
    public Integer deleteCData(String mid) {
        SQLiteDatabase db = this.getWritableDatabase();
        Integer a = db.delete(TABLE_NAME, "PID = ? ", new String[]{mid});
        Cursor res = getAllData();
        txtCountcard.setText("" + res.getCount());
        return a;
    }
